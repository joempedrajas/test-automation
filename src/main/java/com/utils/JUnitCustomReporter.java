...
public class JUnitCustomReporter extends TestListenerAdapter implements IReporter{

	private static final Logger log = Logger.getLogger(JUnitCustomReporter.class);
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String defaultOutputDirectory) {
		...
		for (Map.Entry<Class<?>, Set<ITestResult>> entry : results.entrySet()) {
			...
			log.info("filename :" + getFileName(cls));
			log.info("verbose :" + TestRunner.getVerbose());
			Utils.writeUtf8File(outputDirectory, getFileName(cls), xsb.toXML());
		}

	}

	private String getMethodNameWithParameters(ITestResult result) {
		//Map<String, String> params = result.getTestClass().getXmlTest().getTestParameters();
		Map<String, String> params = result.getMethod().findMethodParameters(result.getTestContext().getCurrentXmlTest());
		String ret = "";
		String displayedName = params.get("displayName");
		if(StringUtils.isNotBlank(displayedName)){
			ret = displayedName;
		}
		else{
			ret = result.getMethod().getMethodName();
		}
		return ret;
	}
